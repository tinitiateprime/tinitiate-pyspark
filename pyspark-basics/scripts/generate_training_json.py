#!/usr/bin/env python3
"""Generate partitioned newline-delimited JSON training data for PySpark examples."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build partitioned JSON files for dept, emp, projects, emp_projects, and salgrade."
    )
    parser.add_argument("--output-dir", default="data/generated_json_10000")
    ...
    parser.add_argument(
        ...
        type=int,
        default=10_000,
        help="Rows per JSON file for every table.",
    )
    parser.add_argument(
        "--max-files",
        type=int,
...
def write_partitioned_json(
    ...
    for part in range(1, file_count + 1):
        ...
        path = table_dir / f"{table_name}_part_{part:04d}.json"
        with path.open("w", encoding="utf-8") as handle:
            for current in range(row_num, row_num + batch_size):
                handle.write(json.dumps(row_factory(current), separators=(",", ":")))
                handle.write("\n")
        row_num += batch_size
...
def main() -> None:
    ...
    write_partitioned_json(
        ...
        args.rows_per_file,
        lambda row_num: dict_from_row(["grade", "losal", "hisal"], salgrade_row(row_num)),
        args.max_files,
    )
