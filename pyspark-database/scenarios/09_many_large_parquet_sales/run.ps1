...
$outputRoot = "data/database_scenarios/09_many_large_parquet_sales"
$scenario = "scenario-09-parquet-large-sales"

Write-Phase 1 "Check prerequisites and capacity"
Assert-TutorialPrerequisites

Write-Phase 2 "Generate large sales Parquet files"
Invoke-SourceGenerator -OutputRoot $outputRoot -Arguments @(
    "--scenario", "parquet-large-sales",
    "--large-file-count", $FileCount,
    "--large-rows-per-file", $RowsPerFile
)

Write-Phase 3 "Inspect expected files and rows"
Show-ScenarioManifest -OutputRoot $outputRoot -ManifestName "parquet-large-sales"

Write-Phase 4 "Load large Parquet files into training.sales"
Invoke-PySparkLoad `
    -SourcePath "$outputRoot/09_parquet_large_sales/sales" `
    -SourceFormat parquet -TargetTable sales -Scenario $scenario `
