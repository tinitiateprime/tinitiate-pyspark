...
$outputRoot = "data/database_scenarios/08_many_small_parquet_multiple_tables"
$scenario = "scenario-08-parquet-small-multi"

Write-Phase 1 "Check prerequisites"
Assert-TutorialPrerequisites

Write-Phase 2 "Generate customer, sales, product, and location Parquet datasets"
Invoke-SourceGenerator -OutputRoot $outputRoot -Arguments @(
    "--scenario", "parquet-small-multi",
    "--small-file-count", $FilesPerTable,
    "--small-rows-per-file", $RowsPerFile
)

Write-Phase 3 "Inspect the multi-table manifest"
Show-ScenarioManifest -OutputRoot $outputRoot -ManifestName "parquet-small-multi"
...
foreach ($table in @("location", "product", "customer", "sales")) {
    ...
    Invoke-PySparkLoad `
        -SourcePath "$outputRoot/08_parquet_small_multi/$table" `
        -SourceFormat parquet -TargetTable $table -Scenario $scenario `
